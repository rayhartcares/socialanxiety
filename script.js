...
function updateNextState(n) {
  ...
  if (GATES[n] && !GATES[n]()) {
    next.disabled = true;
    if (hint) hint.textContent = GATE_HINTS[n] || '';
  } else {
    next.disabled = false;
...
const PW_STORAGE_KEY = 'sal_unlocked_v1';
...
function checkPassword() {
  ...
  if (input.value.trim() === PROGRAM_PASSWORD) {
    try { localStorage.setItem(PW_STORAGE_KEY, '1'); } catch(e) {}
    hideGate();
    launchProgram();
  } else {
...
function init() {
  let unlocked = false;
  try { unlocked = localStorage.getItem(PW_STORAGE_KEY) === '1'; } catch(e) {}
  if (unlocked) {
    launchProgram();
  } else {
    showGate();
