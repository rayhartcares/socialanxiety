...
// ---- Hook into navigateTo to show toasts and update counter ----
...
// ---- Hook S35 into onScreenEnter ----
const _origOnScreenEnter = onScreenEnter;
onScreenEnter = function(n) {
  _origOnScreenEnter(n);
  if (n === 35) buildParentSummary();
};

// ---- Update buildCertificate to include strength statement ----
const _origBuildCert = buildCertificate;
buildCertificate = function() {
  _origBuildCert();
  // Add strength badge if element exists
  const strengthEl = document.getElementById('cert-strength-text');
  if (strengthEl) strengthEl.textContent = getCertStrength();
};
